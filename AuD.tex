...
\brule
Double & Durchlauf beide Richtungen & zusätzlicher Speicher für Referenzen\\
...
on einem Startknoten $S$ werden alle Knoten mit Abstand $k$ durchsucht. Der Suchradius breitet sich aus. Neue Knoten kommen in eine Queue an zu besuchenden Knoten.\\
BREITENSUCHE(v)\\
%TODO Code\\
 \\
...
2\qquad $C_{odd}$ = [C[1], C[3], ..., C[N-1]];\\
3\qquad $C_{even}$ = [C[0], C[2], ..., C[N-2]];\\
4\qquad $R_{odd}$ = FFT($C_{odd}$, $N/2$, $W_N^2$);\\
5\qquad $R_{even}$ = FFT($C_{even}$, $N/2$, $W_N^2$);\\
6\qquad $W = 1$; // = $W_N^0$\\
7\qquad for $l = 0$ to $N/2-1$\\
8\qquad \qquad $R[l] = R_{even}[l] + W * R_{odd}[l]$;\\
9\qquad \qquad $R[l + N/2] = R_{even}[l] - W * R_{odd}[l]$;\\
