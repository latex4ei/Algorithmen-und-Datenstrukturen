...
rekursive Rückführung eines zu lösenden Problems auf mehrere identische Probleme mit kleinerer Eingabemenge\\
...
$g \in \mathcal o (f)$ & $\lim_{n \rightarrow \infty}\fr{g(n)}{f(n)} = 0$ & langsamer als $f$\\
$g \in \omega (f)$ & $\lim_{n \rightarrow \infty}\fr{g(n)}{f(n)} = \infty$ & schneller als $f$\\
\end{tabular}

\subsubsection{Rechenregeln}

$c* f(n) = \mathcal O(f(n))$ für eine beliebige Konst. $c$\\
...
\textbf{Priority Queue:} Queue mit zugeordnetem Schlüssel, Entfernen von Element mit minimalem Schlüssel, Sepicherung in fast vollständigem Binärbaum\\
...
\brule
Verkettete & sequ. Durchlaufen einfach & Zugriff auf i-tes Element\\
Liste & dynamische Länge & zusätzlicher Speicher für Zeiger\\
 & Einfügen/Löschen einfach & \\
\brule
Doppelt & Durchlauf beide Richtungen & zusätzlicher Speicher für Referenzen\\
verkettete & Einfügen/Löschen einfacher & Referenzverw. aufändig\\
Liste & & \\
\end{tabular}
...
\textbf{Laufzeit:} im Mittel loglinear, worst case quadratisch\\
...
\textbf{Laufzeit:} loglinear, \textbf{Rekurrenzgleichung:} $T(n) = 2T(\fr{n}{2}) + O(n)$.
...
\section{Graphen}
% ==========================================================================================================
$G = (E, V)$, mit E: Kante, V: Knoten\\
\textbf{Adjazenzmatrix} $(V \times V)$: $1$ oder $0$ für Verbindung.\\
...
\textbf{Inzident}: Knoten $v$ und Kante $e = (x, v)$ bzw. $e = \{x, v\}$\\
...
\textbf{Breitensuche(BFS)} \\V
on einem Startknoten $S$ werden alle Knoten mit Abstand $k$ durchsucht. Der Suchradius breitet sich aus.\\
BREITENSUCHE(v)\\
1.	queue.enqueue(v);\\
2.	while (!queue.empty())\\
3.		v = queue.dequeue();\\
4.		print(v.val));\\
5.		if (v $\rightarrow$ left) queue.enqueue(v$\rightarrow$left);\\
6.		if (v $\rightarrow$ right) queue.dequeue(v$\rightarrow$right);\\ \\
\textbf{Tiefensuche(DFS)}\\
Laufzeit: $\Theta (\abs V + \abs E)$\\
...
\textbf{Heap-Datenstruktur:} fast vollständiger Binärbaum mit Indizierung von links nach rechts und von oben nach unten.\\
